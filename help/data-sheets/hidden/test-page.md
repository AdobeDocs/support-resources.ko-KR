---
title: 테스트 페이지 (숨김)
description: 내부 테스트용 테스트 페이지
hide: true
hidefromtoc: true
source-git-commit: 26b2032221c1e17ed0a71e556d6acc3f8914dd22
workflow-type: tm+mt
source-wordcount: '1015'
ht-degree: 74%

---

# 테스트 페이지 (숨김)

숨겨진 테스트 페이지

## 이미지 (EXLM-412)

### 호버 텍스트가 있는 이미지

![대체 텍스트 - package.png](assets/package.png "호버 텍스트 - package.png입니다.")

### 텍스트를 가운데로

<p align="center">가운데 위치</p>

<center>가운데 위치</center>

### 확대/축소 가능한 이미지

`![Dive image](assets/maui-dive.jpg "Diving in Maui"){width=100 zoomable}`

**일반**

![다이빙 이미지](assets/maui-dive.jpg "마우이에서의 다이빙"){width=100 zoomable}

**메모**

>[!NOTE]
>
>다이버를 보려면 다음 이미지 클릭:
>
>![다이빙 이미지](assets/maui-dive.jpg "마우이에서의 다이빙"){width=100 zoomable}

**표**

|  | 숫자 | 색상 |
|---|---|---|
| 후아니야 | 17 | 녹색<br>빨간색<br>파란색 |
| 마리아 | 23 | 노란색<br>갈색 |
| Bob | 60 | 이미지 보기<br>![다이빙 이미지](assets/maui-dive.jpg "마우이에서의 다이빙"){width=100 zoomable} |

{style="table-layout:fixed"}

### 기본 이미지 테스트

폭=200 (아래 표와 비교)

![대체 텍스트](assets/maui-dive.jpg "폭 = 200"){width=200}

폭=50% (아래 표와 비교)

![대체 텍스트](assets/maui-flip.jpg "폭 = 50%"){width=50%}

### 줄 바꿈 테스트 ![대체 텍스트](assets/package.png "오른쪽 정렬"){align="right"}

**정렬 없음**

![대체 텍스트](assets/maui-dive.jpg "폭 = 100"){width=100} 마우이에서의 절벽 다이빙은 생각만큼 어렵지 않습니다. 다이빙에는 실제로 네 가지 단계가 있고, 안전한 다이빙에는 다섯 가지 단계가 있습니다. 첫째, 절벽에서 힘차게 뛰어내린 후 즉시 팔을 뻗어 발가락을 가리킵니다. 이는 카메라를 위한 포즈입니다. 둘째, 몸을 적당한 착지 위치로 조종합니다. 이는 패닉처럼 보일 수도 있지만 단지 감정에 따른 조정일 뿐입니다. 셋째, 몸에 상처를 주지 않는 방법으로 입수합니다. 넷째, 이런 일을 이미 백만 번 정도 해 보았다는 듯, 약간은 지루한 표정을 짓습니다.

**오른쪽 정렬**

마우이에서의 절벽 다이빙은 생각만큼 어렵지 않습니다. 다이빙에는 실제로 네 가지 단계가 있고, 안전한 다이빙에는 다섯 가지 단계가 있습니다. 첫째, 절벽에서 힘차게 뛰어내린 후 즉시 팔을 뻗어 발가락을 가리킵니다. 이는 카메라를 위한 포즈입니다. 둘째, 몸을 적당한 착지 위치로 조종합니다. 이는 패닉처럼 보일 수도 있지만 단지 감정에 따른 조정일 뿐입니다. 셋째, 몸에 상처를 주지 않는 방법으로 입수합니다. 넷째, 이런 일을 이미 백만 번 정도 해 보았다는 듯, 약간은 지루한 표정을 짓습니다. ![대체 텍스트](assets/maui-dive.jpg "폭 100 오른쪽 정렬"){width="100" align="right"}

**왼쪽 정렬**

![대체 텍스트](assets/maui-dive.jpg "폭 100 왼쪽 정렬"){width="100" align="left"} 마우이에서의 절벽 다이빙은 생각만큼 어렵지 않습니다. 다이빙에는 실제로 네 가지 단계가 있고, 안전한 다이빙에는 다섯 가지 단계가 있습니다. 첫째, 절벽에서 힘차게 뛰어내린 후 즉시 팔을 뻗어 발가락을 가리킵니다. 이는 카메라를 위한 포즈입니다. 둘째, 몸을 적당한 착지 위치로 조종합니다. 이는 패닉처럼 보일 수도 있지만 단지 감정에 따른 조정일 뿐입니다. 셋째, 몸에 상처를 주지 않는 방법으로 입수합니다. 넷째, 이런 일을 이미 백만 번 정도 해 보았다는 듯, 약간은 지루한 표정을 짓습니다.

**가짜 오른쪽 정렬**

마우이에서의 절벽 다이빙은 어렵지 않습니다. ![대체 텍스트](assets/maui-dive.jpg "폭 100 오른쪽 정렬"){width="100" align="right"}

다이빙에는 실제로 네 가지 단계가 있고, 안전한 다이빙에는 다섯 가지 단계가 있습니다. 첫째, 절벽에서 힘차게 뛰어내린 후 즉시 팔을 뻗어 발가락을 가리킵니다. 이는 카메라를 위한 포즈입니다. 둘째, 몸을 적당한 착지 위치로 조종합니다. 이는 패닉처럼 보일 수도 있지만 단지 감정에 따른 조정일 뿐입니다. 셋째, 몸에 상처를 주지 않는 방법으로 입수합니다. 넷째, 이런 일을 이미 백만 번 정도 해 보았다는 듯, 약간은 지루한 표정을 짓습니다.


### 이미지 정렬

일반

![대체 텍스트](assets/package.png "아이콘에 대한 호버 텍스트")

바로 아래에 정렬

![대체 텍스트](assets/package.png "정렬=오른쪽"){align="right"}

오른쪽 인라인 정렬 ![대체 텍스트](assets/package.png "정렬=오른쪽"){align="right"}

중심 폭 = 250

![대체 텍스트](assets/maui-dive.jpg "정렬=가운데"){align=&quot;center&quot; width=250}

### 제목 정렬

**굵은 제목** ![대체 텍스트](assets/package.png "정렬=오른쪽"){align="right"}

위의 굵은 제목을 참조하십시오.

### 표의 이미지

아이콘은 오른쪽 정렬되고, 다이빙 이미지는 200픽셀로 가운데에, 플립 이미지는 오른쪽 50%에 정렬됩니다.

| <center>왼쪽 | 가운데 | 오른쪽</center> |
|---|---|---|
| ![대체 텍스트](assets/package.png "정렬=오른쪽"){align=right} | ![대체 텍스트](assets/maui-dive.jpg "정렬=가운데 폭=200"){align="center" width="200"} | ![대체 텍스트](assets/maui-flip.jpg "정렬=오른쪽 폭=50%"){align="right" width="50%"} |

## 파일 첨부 (EXLM-1124)

[PDF](/help/data-sheets/assets/BusinessSupportDatasheet.pdf) 다운로드

[JS](assets/main.js) 다운로드

[CSS](assets/main.css) 다운로드

[TXT 파일](assets/dots.txt) 다운로드

[XLSX 파일](assets/4-module_version.xlsx) 다운로드

[ZIP 파일](assets/2-Factor-Authentication-DataSource-and-FDM.zip) 다운로드

## div를 사용한 HTML 테이블

<table>
<tr>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
      <img alt="릴리스 정보" src="assets/package.png" width="40" height="40"/>
    </a>
    <div>
      <a href="/help/data-sheets/business.md"><strong>릴리스 정보</strong></a>
      <p>Adobe Commerce 패치 및 서비스에 대한 모든 릴리스 정보를 검토하십시오.</p>
    </div>
  </td>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
      <img alt="설치" src="assets/package.png" width="40" height="40"/>
    </a>
    <div>
      <a href="/help/data-sheets/business.md"><strong>설치</strong></a>
      <p>온-프레미스 배포용 Adobe Commerce을 설치하는 방법에 대해 알아봅니다.</p>
    </div>
  </td>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
      <img alt="구성" src="assets/package.png" width="40" height="40"/>
    </a>
    <div>
      <a href="/help/data-sheets/business.md"><strong>구성</strong></a>
      <p>Adobe Commerce 애플리케이션에 대한 기능 및 서비스를 구성합니다.</p>
    </div>
  </td>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
      <img alt="데이터 마이그레이션" src="assets/package.png" width="40" height="40"/>
    </a>
    <div>
      <a href="/help/data-sheets/business.md"><strong>데이터 마이그레이션</strong></a>
      <p>Magento 1과 Magento 2 간의 데이터 마이그레이션 프로세스에 대해 알아봅니다.</p>
    </div>
  </td>
</tr>
<tr>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
      <img alt="Upgrade" src="assets/package.png" width="40" height="40"/>
    </a>
    <div>
      <a href="/help/data-sheets/business.md"><strong>업그레이드</strong></a>
      <p>Adobe Commerce 프로젝트를 업그레이드하여 스토어를 안전하게 보호하고 효율적으로 운영하는 방법을 알아봅니다.</p>
    </div>
  </td>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
       <img alt="명령줄 도구 참조" src="assets/package.png" width="40" height="40"/>
    </a>
    <div>
      <a href="/help/data-sheets/business.md"><strong>명령줄 도구 참조</strong></a>
      <p>Adobe Commerce 명령줄 도구의 명령, 인수 및 옵션에 대해 알아봅니다.</p>
    </div>
  </td>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
       <img alt="공연" src="assets/package.png" width="40" height="40"/>
    </a>
    <div>
      <a href="/help/data-sheets/business.md"><strong>성능 모범 사례</strong></a>
      <p>이러한 권장 사항을 사용하여 Adobe Commerce 배포의 성능을 최적화합니다.</p>
    </div>
  </td>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
       <img alt="도구" src="assets/package.png" width="40" height="40"/>
    </a>
    <div>
      <a href="/help/data-sheets/business.md"><strong>도구</strong></a>
      <p>Adobe Commerce에서 사용할 수 있는 도구에 대해 알아봅니다.</p>
    </div>
  </td>
</tr>
<tr>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
      <img alt="구현" src="assets/package.png" width="40" height="40"/>
    </a>
    <div>
      <a href="/help/data-sheets/business.md"><strong>구현 플레이북</strong></a>
      <p>성공적인 Adobe Commerce 사이트에 대한 계획 수립 및 구현을 위한 전략에 대해 알아봅니다.</p>
    </div>
  </td>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
       <img alt="작업" src="assets/package.png" width="40" height="40"/>
    </a>
    <div>
      <a href="/help/data-sheets/business.md"><strong>운영 플레이북</strong></a>
      <p>비즈니스가 성공적인 전자 상거래 사이트를 운영할 수 있도록 하는 방법에 대해 알아봅니다.</p>
    </div>
  </td>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
       <img alt="엔터프라이즈" src="assets/package.png" width="40" height="40"/>
    </a>
    <div>
      <a href="/help/data-sheets/business.md"><strong>규모에 맞는 상거래</strong></a>
      <p>Adobe Commerce와 Adobe Experience Manager를 사용하여 규모에 맞게 경험을 제공하는 방법에 대해 알아봅니다.</p>
    </div>
  </td>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
       <img alt="엔터프라이즈" src="assets/package.png" width="40" height="40"/>
    </a>
    <div>
      <a href="/help/data-sheets/business.md"><strong>보안 및 규정 준수</strong></a>
      <p>Adobe Commerce 판매자가 보안 환경을 유지 관리하는 방법을 알아봅니다.</p>
    </div>
  </td>
</tr>
</table>

## div가 없는 HTML 테이블

<table>
<tr>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
      <img alt="릴리스 정보" src="assets/package.png" width="40" height="40"/>
    </a>
    <p></p>
    <a href="/help/data-sheets/business.md"><strong>릴리스 정보</strong></a>
    <p>Adobe Commerce 패치 및 서비스에 대한 모든 릴리스 정보를 검토하십시오.</p>
  </td>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
      <img alt="설치" src="assets/package.png" width="40" height="40"/>
    </a>
    <p></p>
    <a href="/help/data-sheets/business.md"><strong>설치</strong></a>
    <p>온-프레미스 배포용 Adobe Commerce을 설치하는 방법에 대해 알아봅니다.</p>
  </td>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
      <img alt="구성" src="assets/package.png" width="40" height="40"/>
    </a>
    <p></p>
    <a href="/help/data-sheets/business.md"><strong>구성</strong></a>
    <p>Adobe Commerce 애플리케이션에 대한 기능 및 서비스를 구성합니다.</p>
  </td>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
      <img alt="데이터 마이그레이션" src="assets/package.png" width="40" height="40"/>
    </a>
    <p></p>
    <a href="/help/data-sheets/business.md"><strong>데이터 마이그레이션</strong></a>
    <p>Magento 1과 Magento 2 간의 데이터 마이그레이션 프로세스에 대해 알아봅니다.</p>
  </td>
</tr>
<tr>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
      <img alt="Upgrade" src="assets/package.png" width="40" height="40"/>
    </a>
    <p></p>
    <a href="/help/data-sheets/business.md"><strong>업그레이드</strong></a>
    <p>Adobe Commerce 프로젝트를 업그레이드하여 스토어를 안전하게 보호하고 효율적으로 운영하는 방법을 알아봅니다.</p>
  </td>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
       <img alt="명령줄 도구 참조" src="assets/package.png" width="40" height="40"/>
    </a>
    <p></p>
    <a href="/help/data-sheets/business.md"><strong>명령줄 도구 참조</strong></a>
    <p>Adobe Commerce 명령줄 도구의 명령, 인수 및 옵션에 대해 알아봅니다.</p>
  </td>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
       <img alt="공연" src="assets/package.png" width="40" height="40"/>
    </a>
    <p></p>
    <a href="/help/data-sheets/business.md"><strong>성능 모범 사례</strong></a>
    <p>이러한 권장 사항을 사용하여 Adobe Commerce 배포의 성능을 최적화합니다.</p>
  </td>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
       <img alt="도구" src="assets/package.png" width="40" height="40"/>
    </a>
    <p></p>
    <a href="/help/data-sheets/business.md"><strong>도구</strong></a>
    <p>Adobe Commerce에서 사용할 수 있는 도구에 대해 알아봅니다.</p>
  </td>
</tr>
<tr>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
      <img alt="구현" src="assets/package.png" width="40" height="40"/>
    </a>
    <p></p>
    <a href="/help/data-sheets/business.md"><strong>구현 플레이북</strong></a>
    <p>성공적인 Adobe Commerce 사이트에 대한 계획 수립 및 구현을 위한 전략에 대해 알아봅니다.</p>
  </td>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
       <img alt="작업" src="assets/package.png" width="40" height="40"/>
    </a>
    <p></p>
    <a href="/help/data-sheets/business.md"><strong>운영 플레이북</strong></a>
    <p>비즈니스가 성공적인 전자 상거래 사이트를 운영할 수 있도록 하는 방법에 대해 알아봅니다.</p>
  </td>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
       <img alt="엔터프라이즈" src="assets/package.png" width="40" height="40"/>
    </a>
    <p></p>
    <a href="/help/data-sheets/business.md"><strong>규모에 맞는 상거래</strong></a>
    <p>Adobe Commerce와 Adobe Experience Manager를 사용하여 규모에 맞게 경험을 제공하는 방법에 대해 알아봅니다.</p>
  </td>
  <td valign="top">
    <a href="/help/data-sheets/business.md">
       <img alt="엔터프라이즈" src="assets/package.png" width="40" height="40"/>
    </a>
    <p></p>
    <a href="/help/data-sheets/business.md"><strong>보안 및 규정 준수</strong></a>
    <p>Adobe Commerce 판매자가 보안 환경을 유지 관리하는 방법을 알아봅니다.</p>
  </td>
</tr>
</table>
